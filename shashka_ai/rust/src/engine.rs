...
pub static RC_SQ: Lazy<[[i8; 8]; 8]> = Lazy::new(|| {
    let mut m = [[-1i8; 8]; 8];
    for (i, &(r, c)) in SQ_RC.iter().enumerate() {m[r as usize][c as usize] = i as i8;}
    m
});
...
pub static ZOBRIST: Lazy<([[u64; 32]; 4], u64)> = Lazy::new(|| {
    ...
    let mut t = [[0u64; 32]; 4];
    for c in 0..4 {
        for s in 0..32 {t[c][s] = splitmix64(&mut seed);}
    }
...
pub fn raw_captures(
    ...
    if !king {
        ...
        for d in 0..4 {
            let ray = &rays[d];
            let mut i = 0;
            while i < ray.len() && occ & bit(ray[i]) == 0 {
                i += 1;
            }
            if i >= ray.len() {
                continue;
            ...
            let m = ray[i];
            if enemy & bit(m) == 0 || captured & bit(m) != 0 {
                continue;
            }
            let mut j = i + 1;
...
pub fn has_capture(white: u32, black: u32, sq: u8, my_white: bool, king: bool, captured: u32) -> bool {
    ...
    if !king {
        ...
        for d in 0..4 {
            ...
            let m = ray[i];
            if enemy & bit(m) != 0 && captured & bit(m) == 0 && i + 1 < ray.len() && occ & bit(ray[i + 1]) == 0 {
                return true;
            }
        }
    }
...
pub fn captures_from(p: &Pos, sq: u8, king: bool, my_white: bool, captured: u32) -> Vec<(u8, u8)> {
    ...
    for &(_, c) in &segs {
        if !caps_order.contains(&c) {caps_order.push(c);}
    }
    ...
    for &cap in &caps_order {
        ...
        for &t in &lands {
            ...
            if has_capture(w2, b2, t, my_white, true, new_cap) {
                cont.push(t);
            }
        }
...
pub fn legal_moves(p: &Pos) -> Vec<(u8, u8)> {
    ...
    while bbs != 0 {
        ...
        if !king {
            ...
            for d in dirs {
                if let Some(&t) = rays[d].first() {
                    if occ & bit(t) == 0 {quiets.push((sq, t));}
                }
            }
        } else {
            for d in 0..4 {
...
pub fn apply(p: &mut Pos, action: u16) {
    ...
    if king {p.kings &= !bit(f);}
    if was_man && ((my_white && tr == 0) || (!my_white && tr == 7)) {king = true;}
    if king {p.kings |= bit(t);}
    p.ply += 1;
    if cap >= 0 {
...
pub fn encode(p: &Pos, out: &mut [f32]) {
    ...
    while occ != 0 {
        ...
        out[csq * 6 + cls] = 1.0;
        if p.captured & bit(sq) != 0 {out[csq * 6 + 5] = 1.0;}
    }
    if p.chain_sq >= 0 {
        let cs = (if rot {
            31 - p.chain_sq as u8
